...
export class ListBucketsComponent implements  OnInit  {
  ...
  getObjects(bucketName:string){
    ...
    this.bucketService.getObjects(bucketName).subscribe((data:ObjectList)=>
      {
        this.dataSource = data.s3Objects;
        (console.log(data));
        console.log('ExpandElement  ');console.log(  this.expandedElement);
  ...
  downloadObject(bucketName:string,keyName:string){
    this.bucketService.downloadObject(bucketName,keyName).subscribe(
      (data) =>{
        this._snackBar.open('Download Success. Please check the file at Path C:\\AWS','',{
          duration:4000,
          panelClass:['downloadSuccess']
        });
        console.log(data);
      }, err => {
        this._snackBar.open('Error while download. Please check your Object R/W Permissions','',{
          duration:4000
        });
        console.log(err);
      }
